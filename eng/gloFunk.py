...
def gpDataWriter(allDics, superTokens, strBit, textFile):
    ...
    for all in superTokens:
        ...
        try:
            ...
            gpDic[pWord] = str()        
            for dicEntr in allDics:
                #print(len(dicEntr))
                gpEntr = str()
                try:
                    for keyRing in dicEntr:
                        #print(len(keyRing))
                        gpSet=str()
                        for each in keyRing[pWord]:
                            if (each+'^') not in gpSet:
                                gpSet=gpSet+each+'^'
                                #print(each)
                        #print(pWord, ';', len(keyRing[pWord]), ';', gpSet[:min(30, len(gpSet))])
                        if (len(gpSet) > 0) and (gpSet[-1] != '~'): # to avoid indexError during string slice
                            gpEntr+=(gpSet[:-1])+'~' # -1 to look at the last character, shouldn't be split marker
                        #print(pWord, ';', len(gpEntr), ';', gpSet[:min(30, len(gpSet))])
                except KeyError:
                    print('kE datawriter:', pWord)
                    continue
            gpDic[pWord] = gpEntr
            if len(gpDic[pWord]) > 0:
                print('word:', pWord)
                pFile.writerow([pWord, gpDic[pWord]]) 
                gpDic[pWord] = str() # erases some data to make it less resource-intensive
    #pFile.close()
