...
def popListMaker(empKey, empStr, qAllLines, qLine, qPopSuperList, flowData, rhyList, pLEmps, tagEmpsLine, contrabandQLines): #  Uses the pLine data to generate a list of possible words
...
##            flowData = flowDataRefresh(qLine[0])
    if len(qLine[0]) == 0:
        ...
        for all in proxNumList:
            ...
            print(pLNi)
            proxList = []
            for each in proxPlusLista[all][qLine[1][pLineNList[all]]]:
                if qLine[0]+[each] not in contrabandQLines and qLine[0]+[each] not in supContraLines[empStr]:
            ...
            if len(proxList) == 0:
                ...
                flowData = flowDataRefresh(qLine[0][:-min(pLineNList[all], len(qLine[0]))])
...
def popListDigester(qPopSuperList, qLine, qAllLines, pLEmps, tagEmpsLine, empKey, empStr, rhyList, flowData, contrabandQLines):
    proxNumList, pLNi, pLineNList = flowData
    firstLineLen = len(qLine[0])
    print('firstLine0:', firstLineLen)
...
def wordSubtracter(qLine, qPopSuperList, qAllLines, pLEmps, tagEmpsLine, minusThis, flowData):
    ...
    return qLine, qPopSuperList, qAllLines, flowData, pLEmps, tagEmpsLine
...
def poemLiner(empKey, writQLines, rhyList):
    ...
    while (pLEmps != empKey) and (len(pLEmps) != len(empKey)): # Search for a word that matches the declared meter template, line may need to rhyme (flawed logic here...) and ((len(rhyList)>0) or (pLine[-1] not in rhyList)) and metSwitch == 1 and (pLine[-1] not in quantumList)
        ...
        print('empKey:', empKey)
        if len(qPopSuperList[0][-1]) == 0:
            qPopSuperList, qLine, flowData, pLEmps, tagEmpsLine, qAllLines, contrabandQLines = popListMaker(empKey, empStr, qAllLines, qLine, qPopSuperList, flowData, rhyList, pLEmps, tagEmpsLine, contrabandQLines)
            qPopSuperList[0][-1] = fastTracker(rhyList, qPopSuperList[0][-1])
        qPopSuperList, qLine, flowData, pLEmps, tagEmpsLine, qAllLines, contrabandQLines  = popListDigester(qPopSuperList, qLine, qAllLines, pLEmps, tagEmpsLine, empKey, empStr, rhyList, flowData, contrabandQLines)
...
##                if len(qPopSuperList[0]) == 0:
##                    print('noFirsts')
##                    dynaWords, theReverends = dynaMight(empKey, empStr, theReverends)
##                    if len(dynaWords) == 0:
