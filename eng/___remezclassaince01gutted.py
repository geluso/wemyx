...
def dynaDataWriter(dynasaurus, fileBit, textFile):
    ...
    ##  Writes dynasaurus data to hard drive
...
def dynaMight(wordList, empKey, empStr, pLEmps, superTokens, theReverends): # Grabs possible synonyms, but only ones from the author's lexicon
    ...
    ##  Uses dynasaurus to make a list of synonyms
...
def dynaDataOpener(textFile, dynaType):
    ...
    ##  Opens data saved to dynasaurus
...
def gramLineMaker(xLine, flowData):
    gramLine = []
    xString = gF.lineToString(xLine)  #  Does this line do anything significant?
...
def chopZLine(zLine, qAllLines):  #  Why call this zLine?  What's wrong with existing variables?
    ...
    if len(zLine[0]) == 0:
        ...
        for all in proxNumList: # starts at 0, indexes from the end of line, organized as pLineNList
            ...
            #$print('keepList@:', len(keepList))
                ...
                print('keyFuck', all, proxNumList, pLNi, pLineNList)
                return qPopSuperList, qLine, pLEmps, tagEmpsLine, qAllLines, contrabandQLines, thesClicks
...
def popListDigester(qPopSuperList, qLine, qAllLines, pLEmps, tagEmpsLine, empKey, empStr, rhyList, contrabandQLines, thesClicks):
    ...
    while (len(qPopSuperList[0][-1]) > 0): # and ((len(proxNumList) > proxMinDial) or (len(qLine[0]) <= proxMinDial)): # keep moving forward as long as we keep getting non-empty lists, proxDial restrictions
        ...
        thesList.append(pWord)                  #  What is thesList doing here?
        ...
        if qLine[0]+[pWord] not in supContraLines[empStr] and qLine[0]+[pWord] not in contrabandQLines[0]: # This will screen against trees already explored
            ...
            halfBeatNum = tagEmpsLine.count('')
            print("halvsies", halfSwitch, halfBeatNum)
            if pWord in allPunx:
                pnxCt = 0
                for all in allPunx:
...
##                    if pLEmps[len(qLine)] == '0' and qLine[-1] in quantumWords:
##                        print('quantumShift')
...
##                        qLine = qLine[0][len(qAllLines[0])-1:], qLine[1][len(qAllLines[1])-1:]
                    print('exitY')
                    return qPopSuperList, qLine, pLEmps, tagEmpsLine, qAllLines, contrabandQLines, thesClicks
            elif (halfSwitch == 1) and (halfBeatNum == 0) and (len(qLine[0]) >= 2):
                print("halfBeatCheck", pLEmps, empStr, qLine[0][-2], qLine[0][-1])
                if (qLine[0][-1] in quantumWords) and (qLine[0][-2] in quantumWords) and pLEmps[len(qLine[0]) - 1] == '0':
                    print('halvehit')
                    tagEmpsLine = tagEmpsLine[:-1]
                    tagEmpsLine.append('')
                    # pLEmps = pLEmps[:-1]
                ...
                supContraLines[empStr].append(qLine[0]+[pWord])
                print('contra2:', qLine[0], len(supContraLines[empStr]), '\n', pLEmps, '|', empKey)
                qLine, qPopSuperList, qAllLines, pLEmps, tagEmpsLine, thesClicks = wordSubtracter(qLine, qPopSuperList, qAllLines, pLEmps, tagEmpsLine, 1, thesClicks)
            ...
            if len(thesClicks) > 0:
                if thesClicks[-1] == 0 and theSwitch == 0: # check if user has thesaurus enabled
                    ...
                    while len(thesList) > 0:  # Here's where we start popping items that had previously not worked, to see if it has an effect, thesClicks records one loop
                        print('tList:', thesList, thesClicks)
                        ...
                        try:
                            if thesGLine[-1][0] == 'N':
                                print('dyna-N')
                                for all in supersaurus[thesKey+'.N']:
                                    qPopSuperList[0][-1].append(all)
                            elif thesGLine[-1][0] == 'V':                  #   Should this section have some sort of
                                print('dyna-V')
                                for all in supersaurus[thesKey+'.V']:       #   standardized tags to avoid switching?
                                    qPopSuperList[0][-1].append(all)
                            elif thesGLine[-1][0] == 'R':
                                print('dyna-R')
                                for all in supersaurus[thesKey+'.R']:
                                    qPopSuperList[0][-1].append(all)
                            elif thesGLine[-1][0] == 'J':
                                print('dyna-J')
                                for all in supersaurus[thesKey+'.J']:
                                    qPopSuperList[0][-1].append(all)
                            else:
                                print('thesFail1', thesKey)
                        except KeyError:
                            print('thesFail2:', thesKey)
                            continue
                    if len(qPopSuperList[0][-1]) == 0:
                        print('cut')
                        qLine, qPopSuperList, qAllLines, pLEmps, tagEmpsLine, thesClicks = wordSubtracter(qLine, qPopSuperList, qAllLines, pLEmps, tagEmpsLine, 1, thesClicks)
...
def startWemyx():
    ...
    try:
        ...
        #$for each in proxPlusLista:
            #$print(len(each))
        supersaurus = dynaDataOpener(textFile, 'thes')
...
textChoice.grid(row=13 , column=1, columnspan=2, sticky=W)
...
for each in 'ADJ', 'ADV', 'NOUN', 'VERB':
    ...
    for line in thesFile:
        try:
            ...
            #print('::-> ', thisKey, supersaurus[thisKey])
