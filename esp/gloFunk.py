...
def globalOpen(name, mode):
    ...
    try:
        libFile = csv.reader(open("__global/__data/" + name + "File.csv", 'r+', encoding='latin-1'))
    except IOError:
...
def getLineData(pLine):
    ...
    pLVocs = vocsLine(pLine)
    pLEmps = empsLine(pLine)
    pLFono = fonoLine(pLine)
    pLCons = consLine(pLine)
...
##    except ValueError:
##        continue
##emps=globalOpen('emps', 'lista')
##vocs=globalOpen('vocs', 'lista')
##fono=globalOpen('fono', 'lista')
##cons=globalOpen('cons', 'lista')
##superDic = {}
